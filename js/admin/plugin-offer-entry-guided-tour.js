...
jQuery( 'document' ).ready( function( $ ) {
    ...
    function create_close_button( t , $pointer_options ) {
        ...
        var $btnClose = $( '<button></button>' , {
            ...
            'type': 'button'
        } ).html( jitso_offer_entry_guided_tour_params.texts.btn_close_tour );

        $btnClose.click( function() {

            var data = {
                action : jitso_offer_entry_guided_tour_params.actions.close_tour,
                nonce  : jitso_offer_entry_guided_tour_params.nonces.close_tour,
            };

            $.post( jitso_offer_entry_guided_tour_params.urls.ajax , data , function( response ) {
    ...
    function create_prev_button( t , $pointer_options ) {
        ...
        var $btnPrev = $( '<button></button>' , {
            'class': 'button button-large',
            'type': 'button'
        } ).html( jitso_offer_entry_guided_tour_params.texts.btn_prev_tour );
        ...
        if ( $pointer_options.prev.indexOf( '@' ) >= 0 ) {
            ...
            $btnPrev.click( function() {
                ...
                for ( var i = 0 ; i <= jitso_offer_entry_guided_tour_params.screen.length ; i++ ) {

                    if (  jitso_offer_entry_guided_tour_params.screen[ i ][ 'id' ] == prev_guide_id ) {

                        prev_pointer_options = jitso_offer_entry_guided_tour_params.screen[ i ];
                        break;
    ...
    function create_next_button( t , $pointer_options ) {
        ...
        var text = ( !$pointer_options.prev ) ? jitso_offer_entry_guided_tour_params.texts.btn_start_tour : jitso_offer_entry_guided_tour_params.texts.btn_next_tour;
        ...
        if ( $pointer_options.next.indexOf( '@' ) >= 0 ) {
            ...
            $btnStart.click( function() {
                ...
                for ( var i = 0 ; i <= jitso_offer_entry_guided_tour_params.screen.length ; i++ ) {
                    ...
                    if (  jitso_offer_entry_guided_tour_params.screen[ i ][ 'id' ] == next_guide_id ) {

                        next_pointer_options = jitso_offer_entry_guided_tour_params.screen[ i ];
                        break;
    ...
    function create_buttons( t , $pointer_options ) {
        ...
        var $buttons = $( '<div></div>' , { 'class': 'jitso-tour-buttons' } );
    ...
    function init_offer_entry_guide() {
        ...
        if ( jitso_offer_entry_guided_tour_params.screen.length > 0 )
            init_pointer( jitso_offer_entry_guided_tour_params.screen[0] );
